...
class TfliteService {
  ...
  Future<Map<String, dynamic>?> predictFile(File file) async {
    ...
    img.Image cropped = img.copyCrop(decoded, x: x, y: y, width: size, height: size);
    ...
    return {
      ...
      'confidence': maxScore,
      'isValid': maxScore >= 0.90, // 90% Threshold for professional results
      'all_predictions': Map.fromIterables(_labels, scores),
    };
  }
  ...
  Future<Map<String, dynamic>?> predictFuture(CameraImage image) async {
    ...
    // Use center crop square to ensure aspect ratio isn't squished (which destroys accuracy)
    int cropSize = convertedImg.width < convertedImg.height ? convertedImg.width : convertedImg.height;
    int offsetX = (convertedImg.width - cropSize) ~/ 2;
    int offsetY = (convertedImg.height - cropSize) ~/ 2;
    
    img.Image cropped = img.copyCrop(convertedImg, x: offsetX, y: offsetY, width: cropSize, height: cropSize);
  ...
  img.Image _convertBGRA8888(CameraImage image) {
    return img.Image.fromBytes(
      ...
      rowStride: image.planes[0].bytesPerRow, // Fixes slanted/scrambled images on Windows
